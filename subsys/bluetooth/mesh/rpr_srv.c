...
static void link_report_send_with_cb(const struct bt_mesh_send_cb *cb)
{
...
static inline void link_report_send(void)
{
	link_report_send_with_cb(NULL);
}
		       ...
		       enum prov_bearer_link_status reason)
{
	...
	if (atomic_test_and_clear_bit(srv.flags, NODE_REFRESH)) {
		...
		/*
		 * Use send_cb in order to ensure the link report is sent before
		 * the node is rebooted to swap composition data.
		 */
		link_report_send_with_cb(&refresh_link_close_report_send_cb);
	} else {
