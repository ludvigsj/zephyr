				  ...
				  const struct subnet_pair *expected)
{
	...
	LOG_INF("Getting subnet list, filter = (filter: %d, subnet: %d), start_idx = %d", filter.filter, filter.net_idx, start_idx);
	...
	for (int j = start_idx; j < MAX_EXPECTED_PAIRS && rsp.list->len >= 3; j++) {
		...
		LOG_INF("Received pair (%d, %d)", pair.idx1, pair.idx2);
...
static void test_tester_subnet_duplicate_filtering(void)
{
	...
	/* Add duplicates */
	for (int i = 0; i < ARRAY_SIZE(subnet_pairs); i++) {
		for (int j = 0; j < remote_nodes; j++) {
			bridge_entry_add(PROV_ADDR, DEVICE_ADDR_START + j, subnet_pairs[i].idx1,
					 subnet_pairs[i].idx2, BT_MESH_BRG_CFG_DIR_TWOWAY);
		}
	}
	...
	for (int i = 0; i < ARRAY_SIZE(subnet_duplicate_test_vector); i++) {
		filter = subnet_duplicate_test_vector[i].filter;
		expected = subnet_duplicate_test_vector[i].expected;
